...
func EVAL(code:String):
	...
	if len(code_spaces) == 1:
		return GameState.variables().get_truth(code_spaces[0])
	if len(code_spaces) == 2:
		code_spaces = [code_spaces[0], "=", code_spaces[1]]
	var current_value = GameState.variables().get_string(code_spaces[0])
...
func GV(v:String):
	...
	var val = GameState.variables().get_string(v)
